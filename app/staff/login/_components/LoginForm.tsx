...
export default function LoginForm() {
  ...
  async function handleRequestCode(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/staff-auth/request-code", {
        method: "POST",
      ...
      if (!res.ok && res.status !== 200) {
        // Only surface a generic error for unexpected failures (400, 500)
      ...
      // Move to step 2 regardless of whether the account exists
      setCode("");
      setStep("code");
    } catch {
  ...
  async function handleRequestNewCode() {
    ...
    setIsLoading(true);
    try {
      await fetch("/api/staff-auth/request-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ identifier: identifier.trim() }),
      });
  ...
  async function handleVerifyCode(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/staff-auth/verify-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok && data.success) {
        // Session cookie is set by the API — navigate to the staff area
        ...
        {step === "identifier" && (
          <form onSubmit={handleRequestCode} noValidate>
            <div className="mb-5">
              ...
              />
              {fieldError && (
                <p className="mt-1.5 text-sm text-red-600 font-body">{fieldError}</p>
              )}
            </div>
        ...
        {step === "code" && (
          <form onSubmit={handleVerifyCode} noValidate>
            ...
            <div className="mb-5 rounded-lg bg-emerald-50 px-3.5 py-3 text-sm text-emerald-800 font-body">
              If this staff account exists, a code has been sent.
            </div>
            ...
            <div className="mb-5">
              ...
              <input
                ...
                placeholder="000000"
                className="block w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 tracking-widest placeholder:tracking-widest placeholder:text-slate-400 shadow-xs focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400 font-body"
              />
              {fieldError && (
                <p className="mt-1.5 text-sm text-red-600 font-body">{fieldError}</p>
              )}
            ...
            {/* Secondary actions */}
            <div className="mt-5 flex flex-col items-center gap-2 text-sm font-body">
              <button
                type="button"
                disabled={isLoading}
                onClick={handleRequestNewCode}
                className="text-emerald-700 hover:text-emerald-800 hover:underline disabled:cursor-not-allowed disabled:text-slate-400"
              >
                Request a new code
              </button>
              <button
                ...
                className="text-slate-500 hover:text-slate-700 hover:underline disabled:cursor-not-allowed disabled:text-slate-300"
              >
                Edit email or phone
              </button>
            </div>
